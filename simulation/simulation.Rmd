...
```{r}
proportion_plot <- function(n = n_rep, resolution = 20) {
  ...
  make_em <- function(log_rhr) {
    ...
    # SE equal to that imputed for lot_em.
    list(mean = log_rhr, se = lot_em$se)
  }
...
Run simulations at each of a range of RHRs, estimating the percentage of NMA estimates that would be expected to differ significantly at each value of RHR.

```{r}
proportion_plot_data <- proportion_plot(n = n_rep)
```

Plot the results, with 95% confidence bands.

```{r}
ggplot(proportion_plot_data, aes(x = rhr, y = sig / n)) +
